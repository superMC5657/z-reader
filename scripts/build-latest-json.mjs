 ...
 *     --tag <vX.Y.Z> \
 *     --assets '[{"target":"windows-x86_64","file":"ZReader_0.1.0_x64-setup.exe"}, ...]' \
 *     --dir artifacts \
 *     --output latest.json
 *
 * 说明:
...
if (repo.includes('<') || repo.includes('>')) {
  console.error(
    `错误: --repo 仍是占位符 "${repo}",请替换为真实产物仓库(如 myname/z-reader-release)后再发布`,
  )
  process.exit(1)
}
...
for (const { target, file } of assets) {
  ...
  platforms[target] = {
    signature,
    // 文件名可能含空格(如 "ZReader_0.1.0_x64-setup.exe"),URL 必须百分号编码,
