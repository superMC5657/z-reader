...
> **当前状态**：✅ **已修复并完成落地验证**（已采用「弹窗尺寸隔离 + 步进微调器」方案并上线）
...
### 1. 代码落地位置与实现
1. **弹窗尺寸隔离（[`src/assets/styles.css`](file:///z:/rust_workplace/z-reader/src/assets/styles.css#L420-L455)）**：
   - 显式为 `.modal-mask` 与 `.modal-card` 设置固定基准字号 `font-size: 15px;`；
   - 将弹窗宽度与限制绑定为固定像素 `width: 440px; min-width: 380px; max-width: min(720px, 92vw);`，彻底切断了全局 `--app-font-size` 动态改变对弹窗自身几何尺寸的连锁反馈。
2. **滑块与步进器集成（[`src/components/SettingsPanel.vue`](file:///z:/rust_workplace/z-reader/src/components/SettingsPanel.vue#L282-L315)）**：
   - 滑动条两端配备 `A-` / `A+` 离散步进微调按钮；
   - 提供实时数值徽标 `{{ app.s.fontSize }}px`；
   - 拖拽与点击均可平滑生效。

### 2. 验证结果
- 拖拽滑动条时鼠标相对位置恒定，完全杜绝了数值跳变与 60Hz/120Hz 高频晃动死循环；
- 主界面背景文字实时响应字号放大/缩小，交互体验自然流畅。
